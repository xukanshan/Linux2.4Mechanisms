...
pgd_t pgd[1024] __attribute__((__aligned__(PAGE_SIZE))) __attribute__((__section__(".data.init"))); // 内核页目录表
pte_t pte[1024] __attribute__((__section__(".data.init"))); // 内核第一张页表
...
{
    /* gdt段基址为0x900, 900 908 910 918 920 928 930，没有留出TSS的段描述符*/
    ...
    *((struct gdt_desc *)0x928) = make_gdt_desc((uint32_t *)0, 0xfffff, GDT_DATA_ATTR_LOW_DPL3, GDT_ATTR_HIGH);
    ...
    uint64_t gdt_operand = ((8 * 6 - 1) | ((uint64_t)(uint32_t)0x900 << 16)); // 6个描述符大小
    asm volatile("lgdt %0" : : "m"(gdt_operand));
    asm volatile("call %0" : : "m"(flush));
